...
function Get-VaultTabData_Old {
    ...
    process {
        ...
        if (!$TabName) {
            $AddOns = [FileSystemEntries]::Get($Directory, [FSEntryType]::Directory, [System.IO.SearchOption]::TopDirectoryOnly) 
        }
        else {
            $AddOns = $Directory
        }
...
function Get-VaultAppData_Old {
    ...
    process {
        ...
        if ($TabName) {
            $tools = [FileSystemEntries]::Get($Folder, [FSEntryType]::Directory, [System.IO.SearchOption]::TopDirectoryOnly)
  
        }
        else {
            $tools = [FileSystemEntries]::Get($Folder, [FSEntryType]::Directory, [System.IO.SearchOption]::TopDirectoryOnly) | & { process {
                    [FileSystemEntries]::Get($_, [FSEntryType]::Directory, [System.IO.SearchOption]::TopDirectoryOnly)
                } }
        }
